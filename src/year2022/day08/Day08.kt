...
fun parseTreeMap(input: List<String>): List<List<Int>> {
...
fun List<List<Int>>.isVisible(y: Int, x: Int): Boolean {
    val height = get(y)[x]
...
fun List<List<Int>>.getHighestTreeScenicScore(): Int {
    val maxY = lastIndex
    val maxX = first().lastIndex
...
fun List<List<Int>>.computeTreeScenicScore(y: Int, x: Int): Int {
    val height = get(y)[x]
    var countTop = 0
    var countBottom = 0
    var countLeft = 0
